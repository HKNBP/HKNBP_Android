...
class TVChannel(
    ...
    companion object {
         ...
         * 分析已讀取返來嘅電視頻道表資料
         * */
        private fun parseTVChannels(src: String, onParseedTVChannelsListener: (tvChannels: ArrayLinkList<TVChannel>) -> Unit){
            LoadFile.load(src, fun(xmlHttp){
                ...
                onParseedTVChannelsListener(tvChannels)
            })
        }
        ...
        fun getTVChannels(onLoadedTVChannelsListener: (tvChannels: ArrayLinkList<TVChannel>)->Unit){
            if(tvChannels == null){
                parseTVChannels("data/tv_channels.xml", fun(tvChannels: ArrayLinkList<TVChannel>){
                    try {
                        ...
                        tvChannels.designated(
                                localStorage.getItem("RecentlyWatchedTVChannel")?.toInt()?:
                    ...
                    onLoadedTVChannelsListener(this.tvChannels!!)
                })
            }else{
                onLoadedTVChannelsListener(tvChannels!!)
            }
        }
    }
}
